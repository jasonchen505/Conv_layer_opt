...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaHostRegister((void*) host_output, Batch*Map_out*(Height-K+1)*(Width-K+1)*sizeof(float), cudaHostRegisterDefault);
    cudaHostRegister((void*) host_mask, Map_out*Channel*K*K*sizeof(float), cudaHostRegisterDefault);
    ...
    int stream_num = 2;
    int input_offset = Batch*Channel*Height*Width/stream_num;
    int output_offset = Batch*Map_out*(Height-K+1)*(Width-K+1)/stream_num;
    if(Batch%stream_num != 0)
    {
        stream_num = 1;
    }
    ...
    for(int i = 0;i<stream_num;i++)
    {
        cudaMemcpyAsync((*device_input_ptr)+i*input_offset,host_input+i*input_offset,Batch*Channel*Height*Width/stream_num*sizeof(float),cudaMemcpyHostToDevice,stream[i]);
    }
    const int Height_out = Height - K + 1;
    const int Width_out = Width - K + 1;
    ...
    dim3 DimGrid(Channel,Width_grid*Height_grid,Batch/stream_num);
    dim3 DimBlock(TILE_WIDTH,TILE_WIDTH,1);
    for(int i = 0;i<stream_num;i++)
    {
        matrix_unrolling_kernel<<<DimGrid,DimBlock,0,stream[i]>>>(*device_input_ptr+i*input_offset,unrolled_matrix+i*Batch*Height_out*Width_out/stream_num,Batch,Channel,Height,Width,K);
    }
    ...
    dim3 DimGrid2((Width_unrolled/stream_num+TILE_WIDTH-1)/TILE_WIDTH,(Map_out+TILE_WIDTH-1)/TILE_WIDTH,1);
    ...
    for(int i = 0;i<stream_num;i++)
    {
        matrixMultiplyShared<<<DimGrid2,DimBlock2,0,stream[i]>>>(*device_mask_ptr,unrolled_matrix+i*Width_unrolled/stream_num,matmul_output+i*Width_unrolled/stream_num,Map_out,Height_unrolled,Height_unrolled,Width_unrolled,Map_out,Width_unrolled);
    }
    // matrixMultiplyShared<<<DimGrid2,DimBlock2>>>(*device_mask_ptr,unrolled_matrix,matmul_output,Map_out,Height_unrolled,Height_unrolled,Width_unrolled,Map_out,Width_unrolled);
    // cudaDeviceSynchronize();
    // Permute the result of matrix multiplication
    const int out_image_size = Height_out * Width_out;
    dim3 permute_kernel_grid_dim((out_image_size - 1) / BLOCK_SIZE + 1, Batch/stream_num, 1);
    ...
    for(int i = 0;i<stream_num;i++)
    {
        matrix_permute_kernel<<<permute_kernel_grid_dim, BLOCK_SIZE,0,stream[i]>>>(
            matmul_output+i*Batch*out_image_size/stream_num, *device_output_ptr+i*output_offset, Map_out, Batch, out_image_size
        );
    }
    ...
    for(int i = 0;i<stream_num;i++)
    {
        cudaMemcpyAsync((float*)host_output+i*output_offset,(*device_output_ptr)+i*output_offset,Batch*Map_out*(Height-K+1)*(Width-K+1)/stream_num*sizeof(float),cudaMemcpyDeviceToHost,stream[i]);
    }
    // cudaMemcpy((float*)host_output,*device_output_ptr,Batch*Map_out*(Height-K+1)*(Width-K+1)*sizeof(float),cudaMemcpyDeviceToHost);
    for(int i = 0;i<stream_num;i++)
    {
