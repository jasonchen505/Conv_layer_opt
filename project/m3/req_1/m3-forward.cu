                                   ...
                                   int numCRows, int numCColumns) 
{
    ...
    wmma::fragment<wmma::matrix_b, M, N, K, half, wmma::row_major> fragB;
    wmma::fragment<wmma::accumulator, M, N, K, float> fragC;
    ...
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        ...
        if(ty <= 1)
        {
            ...
            wmma::load_matrix_sync(fragA, &tileA[0][0], 16);
            wmma::load_matrix_sync(fragB, &tileB[0][0], 16);
    ...
    if(ty <= 1)
    {
        wmma::store_matrix_sync(&tileC[0][0],fragC,16,wmma::mem_row_major);
    }
    __syncthreads();
    if (row < numCRows && col < numCColumns) 
    {
...
__global__ void float2half(const float *input, half *output, const size_t size)
{
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // half *unrolled_matrix_fp16;
    // cudaMalloc((void**)&device_mask_fp16,Map_out*Channel*K*K*sizeof(half));
    // cudaMalloc((void**)&unrolled_matrix_fp16,(size_t)Batch*Channel*K*K*Height_out*Width_out*sizeof(half));
    // // Convert the mask and unrolled matrix to half precision
    // float2half<<<(Map_out*Channel*K*K-1)/256+1,256>>>(device_mask,device_mask_fp16,Map_out*Channel*K*K);
    // float2half<<<((size_t)Batch*Channel*K*K*Height_out*Width_out-1)/256+1,256>>>(unrolled_matrix,unrolled_matrix_fp16,(size_t)Batch*Channel*K*K*Height_out*Width_out);
    ...
    matrixMultiplyWMMA<<<DimGrid2,DimBlock2>>>(device_mask,unrolled_matrix,matmul_output,Map_out,Height_unrolled,Height_unrolled,Width_unrolled,Map_out,Width_unrolled);
    cudaDeviceSynchronize();
