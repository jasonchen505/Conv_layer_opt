...
// Fixed point (FP16) arithmetic implementation
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // float2half<<<(Map_out*Channel*K*K-1)/1024+1,1024>>>(device_mask,device_mask_fp16,Map_out*Channel*K*K);
    // float2half<<<((size_t)Batch*Channel*K*K*Height_out*Width_out-1)/1024+1,1024>>>(unrolled_matrix,unrolled_matrix_fp16,(size_t)Batch*Channel*K*K*Height_out*Width_out);
    // cudaDeviceSynchronize();
    ...
    // matrixMultiplyShared<<<DimGrid2,DimBlock2>>>(device_mask_fp16,unrolled_matrix_fp16,matmul_output,Map_out,Height_unrolled,Height_unrolled,Width_unrolled,Map_out,Width_unrolled);
