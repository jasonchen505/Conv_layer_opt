...
// op_0	Weight matrix (Kernel) in constant memory
                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        if (row < numARows && tileId * TILE_WIDTH + tx < numAColumns) {
            tileA[ty][tx] = mask[(size_t) row * numAColumns + tileId * TILE_WIDTH + tx];
        } else {
            tileA[ty][tx] = 0;
        }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    matrixMultiplyShared<<<DimGrid2,DimBlock2>>>(NULL,unrolled_matrix,matmul_output,Map_out,Height_unrolled,Height_unrolled,Width_unrolled,Map_out,Width_unrolled);
    cudaDeviceSynchronize();
